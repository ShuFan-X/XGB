...
N = st.number_input("N", min_value=0, max_value=50, value=1)
L = st.number_input("L", min_value=0, max_value=50, value=1)
Glu = st.number_input("Glu", min_value=0, max_value=50, value=1)
...
if st.button("Predict"):
    ...
    if predicted_class == 1:
        shap.force_plot(explainer_shap.expected_value[1],shap_class1[:,:,1], pd.DataFrame([feature_values], columns=feature_names), matplotlib=True)
    # 期望值（基线值）
    #解释类别 0（未患病）的 SHAP 值
    # 特征值数据
    # 使用 Matplotlib 绘图
    else:
        shap.force_plot(explainer_shap.expected_value[e], shap_class1[:,:,0], pd.DataFrame([feature_values], columns=feature_names), matplotlib=True)
